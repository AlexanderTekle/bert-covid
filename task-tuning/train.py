...
from models import LR, CNN, GRU, BertMLP, RobertaMLP, PretrainedBertMLP
...
def select_model(args):
    ...
    if args.model == 'lr':
        ...
        model = RobertaMLP
    elif args.model == 'pretrainedbert':
        model = PretrainedBertMLP
    else:
...
# Setup vocab.
if args.model == 'bert' or 'pretrainedbert':
    args.n_emb = 30522
elif args.model == 'roberta':
    args.n_emb = 50265
else:
...
if (args.model == 'pretrainedbert' or args.model == 'bert') and args.pretrained_ckpt != '':
    prev_state_dict = torch.load(args.pretrained_ckpt, map_location='cpu')
    for n, p in model.named_parameters():
        if (n in prev_state_dict and n != 'fc.weight' and n != 'fc.bias'):
            w = prev_state_dict[n]
            p.data.copy_(w.data)
        model = model.to(args.device)

# Setup embeddings (optional).
if args.model != 'pretrainedbert' and args.model != 'bert' and args.model != 'roberta':
